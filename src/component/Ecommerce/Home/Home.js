...
const Home = () => {
  ...
  const points = Number(localStorage.getItem("category"));
  const id = localStorage.getItem("id");

  const handlePurchace = (price) => {
    if (points >= price) {
      const newPoints = points - price;
      alert("Product Purchased");
      axios
        .put(`https://65eaab35c9bf92ae3d3be3a5.mockapi.io/userData/${id}`, {
          category: Number(newPoints)
        })
        .then(() => {
          localStorage.setItem('category', newPoints)
        });
      ...
      <nav className="h-[70px] bg-gray-50 w-full flex shadow-md">
        ...
        <div className="w-2/6 flex my-auto justify-between">
          <div className="flex w-full">
            ...
            <div className="text-[25px] ml-2 text-yellow-700">{points}</div>
            <div>
            ...
            <div className="flex float-end">
              <Link to="/">
                <div
                  ...
                  className="bg-gray-300 w-[40px] h-[40px] rounded-full flex justify-center items-center z-10"
                >
                  <GrLogout />{" "}
                </div>
              </Link>
        ...
        <div className="mt-4 grid grid-cols-5 gap-4 max-md:grid-cols-4 max-sm:grid-cols-3 ">
          {prodData.map((item, i) => {
            return (
              <div
                key={i}
                className="w-[250px] h-[400px] bg-gray-100 p-2 shadow-lg rounded-md hover:bg-gray-300"
              >
                <div className="h-3/5">
                  <img
                    src={item.image}
                ...
                <div className="h-2/5">
                  ...
                  <div className="mt-4 flex justify-center items-center">
                    <button
                      onClick={() => handlePurchace(item.points)}
